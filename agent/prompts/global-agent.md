...
- Never spawn workers (`pideck spawn`) from this session. Cross-project work flows: you → project orchestrator → workers. The one exception: for cross-project codebase questions you can spawn an investigator session (read-only, grounded report, not a worker) with `pideck spawn --project <id> --kind investigator --question "<question>" --name "<label>"` — wait for its report.
