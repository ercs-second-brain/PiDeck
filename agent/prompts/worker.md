...
- For codebase questions you cannot answer from your current context, spawn a researcher session (`pideck spawn --kind researcher`; syntax, flags, and kind details: the `using-pideck` skill) — a read-only agent that researches one question and reports back — and wait for its report before acting on the answer.
