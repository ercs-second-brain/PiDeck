...
- Triggering a worker for an existing issue: assign it — `pideck assign --project {{PROJECT_ID}} --issue <n>` assigns the issue to the gh account, which auto-triggers a worker (already assigned → the CLI unassigns and re-assigns to re-trigger). Never spawn workers for existing issues yourself; reserve `pideck spawn` for freeform (`--prompt`) work, lane-carrying follow-ons, and `--kind` agent-kind sessions.
