...
| `pideck assign` | `--project <id>`, `--issue <number>` | `POST /api/projects/:projectId/assign` | Assigns the issue to the daemon's gh account — the worker trigger for existing issues (issue #491): the watcher's `issue.assigned` transition auto-spawns via the issue pipeline; already-assigned issues are unassigned + re-assigned so the re-assignment re-triggers (response reports `retriggered`) |
