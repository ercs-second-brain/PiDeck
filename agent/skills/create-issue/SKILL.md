...
trigger: "Creating or filing a GitHub issue in a pideck project."
---

# Create a GitHub Issue

Creates an issue in the project's GitHub repository via the `gh` CLI. PiDeck does not proxy issue creation through the daemon — the daemon only *watches* issues (see `packages/shared/src/domain.ts`, `Issue`).

## Steps

1. Determine the target repository `OWNER/REPO`:
   - From project state: `pideck project get {{PROJECT_ID}} --json` → `repoUrl` (strip the `https://github.com/` prefix), or
