...
|---|---|---|
| `--session string` | Session id (from `agentskiss sessions`) | Required |
...
agentskiss send --session ask-3 --message "CI on PR #12 is red: typecheck fails in packages/shared. Fix and push."
```

```bash
# Worker escalates a decision to the orchestrator
agentskiss send --session ask-orchestrator --message "Blocked: issue says to edit packages/shared but another worker owns it. How to proceed?"
```
