...
When onboarding finishes, open the webapp at the printed address (`agentskiss addr`) — the sidebar's "+" (or the first-run empty state) launches the onboarding wizard. The wizard now starts with a **pi agent** step (issue #57): it checks the daemon's pi auth probe (`GET /api/pi-auth`), shows the ready providers and configured model, and — when pi is not ready — gives the handoff (run `agentskiss onboard`, or launch pi and use `/login` on the daemon host) and re-verifies before you can continue. pi auth state is also surfaced persistently in the project settings page, in `agentskiss status`, and in `/api/status` (`piReady`); the daemon logs a warning at startup when no provider is ready. Then connect a repo: clone from git or create a new GitHub repo (private by default, toggle for public), and choose whether issues auto-create agents — registering the project opens its kanban board. From the sidebar you can then click a project to reopen its board, click the **Projects** header for the combined all-projects board, or click an agent row to talk to the project's orchestrator (if the project doesn't have one yet, hit **Start orchestrator** in the sidebar and it comes up immediately).
