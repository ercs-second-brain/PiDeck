...
- **`apps/web/`** — the webapp the daemon serves: one single-page terminals app (issue #62) whose sidebar is the whole navigation — a "Projects" header (click it for the all-projects combined kanban) with a "+" button that launches the onboarding wizard (pi/gh status, clone-or-create repo with private-by-default) as a modal, per-project rows that open the project's kanban board in the main pane, and per-agent rows that attach terminals — the orchestrator first with its workers indented beneath it (issue #63) and live worker status badges; active worker rows carry a terminate affordance (✕ → confirm) and terminated workers move to a collapsed per-project "Archived" section (issue #64). PR diff review and project settings open in the same main pane.
...
**Status of the automated loop:** the full PRD core loop is live — orchestrator sessions, worker spawning (manual via webapp / orchestrator chat / `pideck spawn`, **and automatic via the GitHub issue watcher**), kanban tracking of issues and PRs with CI/review state, **CI-failure auto-fix**, **review-comment addressing**, diff review, browser terminals, session persistence across daemon restarts. Spawning is assignment-driven (issue #416, default — no setting): the daemon polls GitHub and assigning any user to an issue spawns a worker automatically (unless blocked by native blocked-by links, capped by the project's `workerConcurrency`); unassigning or closing the issue cancels any queued spawn and archives the issue's workers; a worker PR that goes red gets CI-fix prompts (bounded retries); new review comments are delivered to the worker. Polling runs at a 30s default interval (API-budget notes in `apps/daemon/src/pipeline/wiring.ts`); tune or disable it with `PD_WATCHER_POLL_INTERVAL_MS` / `PD_WATCHER_ENABLED=0`.

**Review account (the review leg, issue #407):** the loop's review leg runs as a **second GitHub account**, configured in the webapp (sidebar footer → **Global settings** → *Review account*) with that account's username + personal access token. When both are set, auto review agents spawn on green, unapproved PRs and file real GitHub reviews as the review account (`gh pr review` under its own `GH_TOKEN`) — reviews the primary account's worker pipeline then reacts to. With only the primary account configured there is **no review leg at all**: one account cannot review its own PRs, so the PR loop stays worker + CI only. Username and token are saved together (both-or-neither, enforced server-side); the token is stored in the daemon settings file (`<stateDir>/settings.json`, same plaintext posture as gh's own `hosts.yml`) and **masked on read** (issue #428): the API and the settings UI never return or display it — you can replace or clear it, but the stored value is never shown.
...
The PRD success metric — *issue created → worker assigned → worker spawns → PR → CI breaks → worker fixes → CI green, zero manual commands* — holds for a connected project with a working `gh` login; CI/review follow-up is bounded (`maxFixAttempts`), after which a human takes over.
