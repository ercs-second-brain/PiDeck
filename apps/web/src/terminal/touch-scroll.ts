 ...
 * xterm's scrollback like desktop mouse-wheel scrolls do.
 *
 * Root cause of the reported "scroll up doesn't work on mobile": xterm 5.5's
 * built-in touch path (Viewport.handleTouchMove) scrolls by writing the
 * hidden overflow box's scrollTop, then still asks the browser to pan via
 * the event default (`_bubbleScroll` preventDefaults while the browser has
 * already claimed the vertical pan for its own native scroll). The claimed
 * gesture dies at the compositor — the stream is cancelled after the first
 * move — so a swipe moves about one line and stops, which reads as "scroll
 * doesn't work". Upstream fixed exactly this class of break in xterm 6.0
 * (commit 38913e4a "Fix scrolling with touch") by taking the gesture over at
 * the JS level: claim it from touchstart, preventDefault so the browser
 * never pans, and scroll the viewport from touch deltas. This controller is
 * that takeover, scoped to the app (xterm 5.5 cannot be upgraded here).
 *
 * Composes with the mobile input gate (issue #374): the textarea focus on
 * tap is parity-only (the OS keyboard stays locked out of the gated
 * textarea), and the coarse-pointer flag that installs this controller is
 * the same flag that gates the composer. Mechanisms are distinct — this
 * touches only the viewport's scroll path, never the input path.
 */

/**
 * Structural slice of the DOM the controller needs — kept minimal so tests
 * can pass stubs instead of a live xterm surface (same convention as the
 * mobile input gate's GatableTextarea).
 */
 ...
 * touch-like events with no lists at all (some engines emit compat events
 * whose shape varies mid-gesture) — those contribute no fingers.
 */
function touchesOf(
