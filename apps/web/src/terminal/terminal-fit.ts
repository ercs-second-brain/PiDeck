/**
 * Fit/resize wiring for the browser terminal (issues #124 and #353): turns
 * xterm's fit addon measurements into `terminal.resize` frames on the daemon
 * so the tmux window (and therefore the pane's rendering width) follows the
 * browser viewport.
 *
 * Extracted from `TerminalPane` so the propagation rules are unit-testable
 * without a DOM:
 * - the first invocation fits synchronously — `TerminalPane` attaches with
 *   the terminal's size right after wiring the observer, so the initial fit
 *   must not be deferred;
 * - subsequent invocations are coalesced into one trailing fit: a container
 *   that changes every animation frame (sidebar collapse animation, pane
 *   drag-resize) would otherwise push an intermediate size to tmux on every
 *   frame — each one a tmux resize plus full screen repaint, and a
 *   client-side rewrap of the scrollback at a width that is stale again on
 *   the next frame (issue #353's "wrapping is messed up while resizing");
 ...
 *   daemon;
 * - `dispose()` cancels a pending trailing fit — pane teardown must not fit
 *   a disposed terminal.
 */
...
export interface FitControllerOptions {
  ...
  /**
   * Coalescing window (ms) for resize bursts. 0 disables the debounce
   * (every invocation fits immediately). Default 150.
   */
  debounceMs?: number;
}
...
export function createFitController(options: FitControllerOptions): FitController {
  ...
  const controller = (() => {
    if (firstCall) {
      // The pane attaches with the terminal's size immediately after wiring
      // the observer — the initial fit must land synchronously.
      firstCall = false;
      run();
      return;
    }
