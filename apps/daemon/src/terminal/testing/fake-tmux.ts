/**
 * In-memory fake `TmuxRunner` simulating a tmux server, for the terminal
 * bridge tests.
 *
 * Copied from `apps/daemon/src/sessions/testing/fake-tmux.ts` (issue #4)
 * and extended with the commands the terminal bridge uses:
 * - `send-keys -H <hex>...` — hex-encoded bytes are decoded and appended to
 *   the pane like the literal mode does.
 * - `capture-pane` tolerates the `-e` flag (escape-sequence capture) and
 *   returns the seeded pane lines verbatim.
 *
 * Only implements the subset of commands the daemon uses.
 */

import { TmuxError, type CommandResult, type TmuxRunner } from "../../sessions/tmux.js";
...
export class FakeTmuxRunner {
  ...
  private execute(args: string[]): string {
    ...
    switch (cmd) {
      ...
      case "capture-pane": {
        ...
        // `-p`/`-e` are output-mode flags the fake renders the same way;
        // `-S`/`-E` history bounds are ignored (all seeded lines returned).
      ...
      case "send-keys": {
        ...
        if (hex !== -1) {
          pane.paneLines.push(this.decodeHex(cmdArgs.slice(hex + 1)));
        }
        return "";
      }
      default:
  ...
  /** Decodes `send-keys -H` hex byte arguments into a literal string. */
  private decodeHex(hexArgs: string[]): string {
    let text = "";
    for (const arg of hexArgs) {
      if (!/^[0-9a-fA-F]{2}$/.test(arg)) this.fail(`bad hex byte: ${arg}`, []);
