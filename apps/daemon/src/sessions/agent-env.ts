 ...
 *
 * The values are injected per-session with `tmux new-session -e` (see
 * {@link Tmux.newSession}): a pane otherwise inherits the tmux SERVER's
 * global environment — captured when that server first started — so on a
 * long-lived user tmux server every new session would keep running pi
 * under a stale PATH (e.g. the pre-update system Node) even after the
 * daemon restarted on the fresh runtime.
 */
