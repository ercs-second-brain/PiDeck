...
export class SessionManager {
  ...
  async ensureOrchestrator(projectId: string): Promise<Session> {
    return this.ensureOrchestratorSession(projectId, () => {
      this.ensureProject(projectId);
      return this.layout.projectDir(projectId);
    });
  }
  ...
  async ensureGlobalAgent(): Promise<Session> {
    return this.ensureOrchestratorSession(GLOBAL_AGENT_PROJECT_ID, () => {
      mkdirSync(this.layout.root, { recursive: true });
   ...
   * session in the cwd the `prepare` callback resolves.
   */
  private async ensureOrchestratorSession(projectId: string, prepare: () => string): Promise<Session> {
