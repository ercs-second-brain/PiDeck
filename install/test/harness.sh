# shellcheck shell=sh
#
# Shared test harness for install/test/*.sh (no bats dependency; matches
# install/'s shell-only tooling). Provides the script paths (all test scripts
# live next to this file), a temp dir with cleanup, and the tiny check_*
# assertion helpers. Source it right after `set -u`; exit with "$failures".
