...
# this monorepo; when release artifacts become available, replace ONLY this
# function — the rest of the installer already consumes the well-known layout
# it must keep producing:
#
#   $PD_SRC/apps/daemon/dist/index.js   (daemon entry)
#   $PD_SRC/apps/daemon/dist/cli.js     (daemon CLI the pideck shim forwards to)
#   $PD_SRC/apps/web/dist/…             (webapp assets)
#   $PD_SRC/agent/skills/…              (shipped pi skills)
#
