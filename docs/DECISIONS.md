...
| 22 | Worker blockers are issue comments starting with `BLOCKED:`; delivery routing is by content, not author | Worker and orchestrator both act through the primary account, so authorship cannot distinguish direction; a prefix the daemon matches keeps the blocker path GitHub-native. |
| 23 | pi runs with `--session-dir <stateDir>/pi-sessions/<sessionId>` | Pins pi's session storage to a path PiDeck owns, so the context probe reads the session JSONL without reconstructing pi's internal cwd-slug layout. |
| 24 | The review account must be a collaborator on every registered repo, and the daemon manages that access | `gh pr review` needs push-level access; the daemon checks access on registration and every poll, invites with push as the primary account, and accepts the invitation as the review account. No reviewer spawns while access is missing. |
...
| 29 | The review account is required everywhere — no leg-off mode in the daemon, fixtures, or docs | One workflow; head-matched approval makes merge safety exact; onboarding already enforced it. The last leg-off fixtures (fake gh, the e2e harness, the gallery seeds) are gone. Migration: an install still on `reviewAccount: null` is not broken — the loop stays idle and logs once until `pideck onboard` completes; the account can no longer be cleared through the API. |
